...
class CallAcceptTipView: UIView {
    ...
    static func showTipView(_ type: CallType, userInfo: UserInfo) -> CallAcceptTipView {
        ...
        switch type {
        case .voice:
            tipView.messageLabel.text = ZGLocalizedString("zego_voice_call",tableName: CallUIKitTable)
            tipView.acceptButton.setImage(UIImage(named: "call_accept_icon"), for: .normal)
        case .video:
            tipView.messageLabel.text = ZGLocalizedString("zego_video_call",tableName: CallUIKitTable)
